 ...
 *
 */
//prng_num prng_cmrg_get_next_int(struct cmrg_state *g)
inline prng_num prng_cmrg_get_next_int_adapted_mul(struct cmrg_state *g)
{
  int h, p12, p13, p21, p23;
  int condition;
  ...
  condition = p13 < 0;
  p13 += 2147483647U*condition;
  //if (p13 < 0) p13 = p13 + 2147483647U;
  condition = p12 < 0;
  p12 += 2147483647U*condition;
  //if (p12 < 0) p12 = p12 + 2147483647U;

  g->x10 = g->x11; g->x11 = g->x12; g->x12 = p12 - p13;

  condition = g->x12 < 0;
  g->x12 += 2147483647U*condition;
  //if (g->x12 < 0) g->x12 = g->x12+2147483647U;
  ...
  condition = p23 < 0;
  p23 += 2145483479*condition;
  //if (p23 < 0) p23 = p23 + 2145483479;
  condition = p21 < 0;
  p21 += 2145483479U*condition;
  //if (p21 < 0) p21 = p21 + 2145483479U;

  g->x20 = g->x21; g->x21 = g->x22; g->x22 = p21 - p23;

  condition = g->x22 < 0;
  g->x22 += 2145483479U*condition;
  //if (g->x22 < 0) g->x22 = g->x22+2145483479U;

  /* Combination */
  asm("cmp %0,%1,%2": "=r" (condition): "r" (g->x22), "r" (g->x12));
  condition = condition < 0;
  return g->x12 - g->x22 + 2147483647U*condition;
  /*
  if (g->x12 < g->x22)
    return(g->x12 - g->x22 + 2147483647U);
  else
...
int main() {
	...
	g.x22 = 1327362106;
/*
	g_original.x10 = -1155484576;
	g_original.x11 =-723955400;
	g_original.x12 = 1033096058;
	g_original.x20 = -1690734402;
	g_original.x21 = -1557280266;
	g_original.x22 = 1327362106;
	*/
/*
	for(i=0; i<N; i++) {
		prng_cmrg_get_next_int(&g_original);
	}
*/
	for(i=0; i<N; i++) {
		prng_cmrg_get_next_int_adapted_mul(&g);
	}
/*
 	for(i=0; i<it; i++) {
...
*/
/*
	if(g.x10 != g_original.x10) {
		while(1);
	}
	if(g.x11 != g_original.x11) {
		while(1);
	}
	if(g.x12 != g_original.x12) {
			while(1);
	}
	if(g.x20 != g_original.x20) {
			while(1);
	}
	if(g.x21 != g_original.x21) {
			while(1);
	}
	if(g.x22 != g_original.x22) {
			while(1);
	}
*/
	return 0;
