...
*/
void IMG_mad_8x8_c_adapted_mul
(
    const unsigned char *refImg,
    const unsigned char *srcImg,
    int pitch, int sx, int sy,
...
)
{
    ...
    for (x = 0; x < sx; x++)
        for (y = 0; y < sy; y++)
        {
            ...
            for (i = 0; i < 8; i++)
                for (j = 0; j < 8; j++) {
                    temp = srcImg[i*8 + j] - refImg[(i+y)*pitch + x + j];
					// ABS
					condition = temp < 0;
...
//    unsigned int motvec_original[2];
...
(
    refImg,
    srcImg,
    pitch, sx/factor, sy/factor,
    motvec_original
);
*/
IMG_mad_8x8_c_adapted_mul
(
    refImg,
    srcImg,
    pitch, sx/factor, sy/factor,
