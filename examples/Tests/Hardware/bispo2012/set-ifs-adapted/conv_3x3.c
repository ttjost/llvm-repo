...
const unsigned char inptr[X_DIM*3] = {
    ...
    79, -51, -107, 30, 60, -128, 114, 58, -60, -114, 21, -55, 74, 14, -28
};
...
void IMG_conv_3x3_c_adapted_mul(const unsigned char *inptr,
                 unsigned char       *outptr,
                 int                 x_dim,
                 const char          *mask,
                 int                 shift)
{
	...
	int condition1, condition2, temp;
     ...
     for (j = 0; j < x_dim ; j++)
     {
		 ...
		 condition1 = sum < 0;
		 temp = 255;
		 asm("cmp %0,%1,%2": "=r" (condition2): "r" (temp), "r" (sum));
		 condition2 = condition2 > 0;

		 sum = 0*condition1 | 255*condition2 | sum*(!condition1 & !condition2);
		 //sum = 0*condition1 + 255*condition2 + sum*(!condition1 & !condition2);
...
//	int i;
                 ...
                 //unsigned char       outptr_original[X_DIM];

                 int                 x_dim = X_DIM;
                 int                 shift = 0;
/*
   IMG_conv_3x3_c(inptr,
                 outptr_original,
                 x_dim,
                 mask,
                 shift);
*/
   IMG_conv_3x3_c_adapted_mul(inptr,
                 outptr,
                 x_dim,
                 mask,
                 shift);
