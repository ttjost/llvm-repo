...
Xuint8 imageR_original[N_ELEMS] = {
    57, -8, -112, 23, -22, -51, 92, 78, 15, 121, -100, -67, -12, -20, 36,
    64, -73, 30, -123, 94, 44, 31, -114, 117, 1, 109, 21, 55, 34, -123,
    -9, 84, 94, -68, 92, 120, 74, 78, 63, 120, 101, -73, 45, -57, 124,
    65, 76, 46, 80, -34, -98, -68, 117, 55, -121, -6, 124, 4, -116, 99,
...
void  compositing_adadpted_mul(Xuint8 *image1, Xuint8 *image2, Xuint8 *imageR, int nelem, Xuint8 fade)
{
  int i;
  int condition, temp;
  for (i=0; i < nelem; i++, image1++, image2++, imageR++) {
    ...
    temp = 255;
    asm("cmp %0,%1,%2": "=r" (condition): "r" (temp), "r" (T)) ;
    condition = condition > 0;
    T = 255*condition | T*!condition;
    //T = 255*condition + T*!condition;
    //if (T > 255) T = 255;
    *imageR = (Xuint8) T;
  }
...
/*
	for(i=0; i<N_FADES; i++) {
		compositing(image1, image2, imageR_original, N_ELEMS, fades[i]);
	}
*/
	for(i=0; i<N_FADES; i++) {
		compositing_adadpted_mul(image1, image2, imageR, N_ELEMS, fades[i]);
	}
