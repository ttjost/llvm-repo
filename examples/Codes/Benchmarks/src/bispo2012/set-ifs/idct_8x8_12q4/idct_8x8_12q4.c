...
int main() {
   ...
   unsigned num_idcts = NUM_IDCTS;

   IMG_idct_8x8_12q4_c(idct_data, num_idcts);


   short (*idct_test)[8][8] = (short (*)[8][8])idct_data;
   int i,j,l;
   for(i = 0; i < num_idcts; i++)
     {
       for (j = 0; j < 8; j++)
	 {
	   for (l = 0; l < 8; l++)
	     {
	       if(idct_test[i][l][j] != idct_out_ref[i][l][j])
		 {
#ifdef C
		   printf("%d, idct  %d, line  %d, colum  %d\n", idct_test[i][l][j], i, l, j);
#endif
		   return 666;
		 }
	     }
