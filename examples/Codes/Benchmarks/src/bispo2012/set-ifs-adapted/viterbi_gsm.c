...
short sd[2*N+1] = {
 ...
 // ouput
 short new_s_original[24];
 short trans_original[N];
...
void vitgsm_adapted_mul(int n, short old[], short new_s[], short trans[], short m[], short sd[]) {
  int tmp1;
  int   i, j, k, a, b, m0, m1, mj;
  ...
  for (i = 0; i < n; i++) {
	...
	asm("cmp %0,%1,%2": "=r" (condition): "r" (a), "r" (b)) ;
	condition = condition > 0;
	new_s[j + k] = b*condition | a*!condition;
	//new_s[j + k] = b*condition + a*!condition;
	//new_s[j + k] = (b > a) ? b : a;


	trans[i] = (trans[i] << 1) | (condition);
	//trans[i] = (trans[i] << 1) | (b > a);
	mj = -mj;
      }
    }
...
int main() {
 ...
 //vitgsm(n, old_original, new_s_original, trans_original, m_original, sd);
 vitgsm_adapted_mul(n, old, new_s, trans, m, sd);
/*
 for(i=0; i<24; i++) {
	if(new_s_original[i] != new_s[i]) {
		while(1);
	}
 }

 for(i=0; i<N; i++) {
	if(trans_original[i] != trans[i]) {
		while(1);
	}
 }
*/
