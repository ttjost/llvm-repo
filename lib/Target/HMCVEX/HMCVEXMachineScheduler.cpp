//===- HMCVEXMachineScheduler.cpp - MI Scheduler for HMCVEX -------------===//
//
...
void HMCVEXVLIWMachineScheduler::postprocessDAG() {
...
/// empirically.
bool HMCVEXVLIWResourceModel::isResourceAvailable(SUnit *SU) {
  if (!SU || !SU->getInstr())
    return false;
  ...
  case HMCVEX::ADJCALLSTACKUP:
  case HMCVEX::ADJCALLSTACKDOWN:
    break;
  }
...
bool HMCVEXVLIWResourceModel::reserveResources(SUnit *SU) {
...
void HMCVEXVLIWMachineScheduler::schedule() {
  DEBUG(dbgs()
...
void ConvergingHMCVEXVLIWScheduler::initialize(ScheduleDAGMI *dag) {
  DAG = static_cast<HMCVEXVLIWMachineScheduler*>(dag);
  ...
  Top.ResourceModel = new HMCVEXVLIWResourceModel(TM, DAG->getSchedModel());
  Bot.ResourceModel = new HMCVEXVLIWResourceModel(TM, DAG->getSchedModel());

  assert((!llvm::ForceTopDown || !llvm::ForceBottomUp) &&
         "-misched-topdown incompatible with -misched-bottomup");
}

void ConvergingHMCVEXVLIWScheduler::releaseTopNode(SUnit *SU) {
  if (SU->isScheduled)
    return;
...
void ConvergingHMCVEXVLIWScheduler::releaseBottomNode(SUnit *SU) {
  if (SU->isScheduled)
    return;
...
bool ConvergingHMCVEXVLIWScheduler::SchedBoundary::checkHazard(SUnit *SU) {
...
void ConvergingHMCVEXVLIWScheduler::SchedBoundary::releaseNode(SUnit *SU,
                                                     unsigned ReadyCycle) {
...
void ConvergingHMCVEXVLIWScheduler::SchedBoundary::bumpCycle() {
...
void ConvergingHMCVEXVLIWScheduler::SchedBoundary::bumpNode(SUnit *SU) {
  bool startNewCycle = false;
...
void ConvergingHMCVEXVLIWScheduler::SchedBoundary::releasePending() {
...
void ConvergingHMCVEXVLIWScheduler::SchedBoundary::removeReady(SUnit *SU) {
...
SUnit *ConvergingHMCVEXVLIWScheduler::SchedBoundary::pickOnlyChoice() {
  if (CheckPending)
    releasePending();
...
#ifndef NDEBUG
void ConvergingHMCVEXVLIWScheduler::traceCandidate(const char *Label,
                                             const ReadyQueue &Q,
...
int ConvergingHMCVEXVLIWScheduler::SchedulingCost(ReadyQueue &Q, SUnit *SU,
...
ConvergingHMCVEXVLIWScheduler::CandResult ConvergingHMCVEXVLIWScheduler::
...
SUnit *ConvergingHMCVEXVLIWScheduler::pickNodeBidrectional(bool &IsTopNode) {
...
SUnit *ConvergingHMCVEXVLIWScheduler::pickNode(bool &IsTopNode) {
...
//         I->getSUnit()->dump(DAG);
        ...
        if (SU->getInstr()->getOpcode() == HMCVEX::CALL) {
          ...
          if (tempInstr->getNumOperands() != 0) {
            ...
            if (tempOperand.isReg())
              if (tempOperand.getReg() == HMCVEX::Lr) {
...
/// that was just returned by pickNode(). However, HMCVEXVLIWMachineScheduler needs
/// to update it's state based on the current cycle before MachineSchedStrategy
/// does.
void ConvergingHMCVEXVLIWScheduler::schedNode(SUnit *SU, bool IsTopNode) {
  if (IsTopNode) {
