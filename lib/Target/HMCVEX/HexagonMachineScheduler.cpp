...
extern cl::opt<bool> DisableHMCVEXMISched;
...
/// empirically.
bool NewHMCVEXVLIWResourceModel::isResourceAvailable(SUnit *SU) {
  if (!SU || !SU->getInstr())
    return false;
...
bool NewHMCVEXVLIWResourceModel::reserveResources(SUnit *SU) {
...
void NewHMCVEXVLIWMachineScheduler::schedule() {
  DEBUG(dbgs()
...
void NewHMCVEXConvergingVLIWScheduler::initialize(ScheduleDAGMI *dag) {
  DAG = static_cast<NewHMCVEXVLIWMachineScheduler*>(dag);
  ...
  Top.ResourceModel = new NewHMCVEXVLIWResourceModel(STI, DAG->getSchedModel());
  Bot.ResourceModel = new NewHMCVEXVLIWResourceModel(STI, DAG->getSchedModel());

  assert((!llvm::ForceTopDown || !llvm::ForceBottomUp) &&
         "-misched-topdown incompatible with -misched-bottomup");
}

void NewHMCVEXConvergingVLIWScheduler::releaseTopNode(SUnit *SU) {
  if (SU->isScheduled)
    return;
...
void NewHMCVEXConvergingVLIWScheduler::releaseBottomNode(SUnit *SU) {
  if (SU->isScheduled)
    return;
...
bool NewHMCVEXConvergingVLIWScheduler::NewHMCVEXVLIWSchedBoundary::checkHazard(SUnit *SU) {
...
void NewHMCVEXConvergingVLIWScheduler::NewHMCVEXVLIWSchedBoundary::releaseNode(SUnit *SU,
                                                     unsigned ReadyCycle) {
...
void NewHMCVEXConvergingVLIWScheduler::NewHMCVEXVLIWSchedBoundary::bumpCycle() {
...
void NewHMCVEXConvergingVLIWScheduler::NewHMCVEXVLIWSchedBoundary::bumpNode(SUnit *SU) {
  bool startNewCycle = false;
...
void NewHMCVEXConvergingVLIWScheduler::NewHMCVEXVLIWSchedBoundary::releasePending() {
...
void NewHMCVEXConvergingVLIWScheduler::NewHMCVEXVLIWSchedBoundary::removeReady(SUnit *SU) {
...
SUnit *NewHMCVEXConvergingVLIWScheduler::NewHMCVEXVLIWSchedBoundary::pickOnlyChoice() {
  if (CheckPending)
    releasePending();
...
#ifndef NDEBUG
void NewHMCVEXConvergingVLIWScheduler::traceCandidate(const char *Label,
                                             const ReadyQueue &Q,
...
int NewHMCVEXConvergingVLIWScheduler::SchedulingCost(ReadyQueue &Q, SUnit *SU,
...
NewHMCVEXConvergingVLIWScheduler::CandResult NewHMCVEXConvergingVLIWScheduler::
...
SUnit *NewHMCVEXConvergingVLIWScheduler::pickNodeBidrectional(bool &IsTopNode) {
...
SUnit *NewHMCVEXConvergingVLIWScheduler::pickNode(bool &IsTopNode) {
...
/// that was just returned by pickNode(). However, NewHMCVEXVLIWMachineScheduler needs
/// to update it's state based on the current cycle before MachineSchedStrategy
/// does.
void NewHMCVEXConvergingVLIWScheduler::schedNode(SUnit *SU, bool IsTopNode) {
  if (IsTopNode) {
