//===-- HMCVEXVLIWPacketizer.cpp - HMCVEX VLIW Packetizer     -------------------===//
//
...
#define DEBUG_TYPE "HMCVEX-vliw-packetizer"

using namespace llvm;

namespace llvm {
    FunctionPass *createHMCVEXPacketizer(bool EnableVLIWScheduling, HMCVEXTargetMachine &TM);
}

namespace {
class HMCVEXPacketizer : public MachineFunctionPass {
...
public:
    ...
    HMCVEXPacketizer(TargetMachine &TM,
...
class HMCVEXPacketizerList : public VLIWPacketizerList {
    ...
    const HMCVEXInstrInfo *HMCVEXII;
    const HMCVEXSubtarget* Subtarget;
    const InstrItineraryData *II;
    TargetMachine &TM;

public:
    HMCVEXPacketizerList(TargetMachine &TM,
                      MachineFunction &MF,
                      MachineLoopInfo &MLI)
                      : TM(TM),
                        VLIWPacketizerList(MF, MLI, true) {
        HMCVEXII = (const HMCVEXInstrInfo *) TII;
        Subtarget = &MF.getSubtarget<HMCVEXSubtarget>();
        II = static_cast<const HMCVEXSubtarget *>(Subtarget)->getInstrItineraryData();
        DataHazards.clear();
    }
...
bool HMCVEXPacketizerList::isLongImmediate(int64_t Immediate) {
...
bool HMCVEXPacketizerList::canReserveResourcesForLongImmediate (MachineBasicBlock::iterator MI) {

    MachineFunction *MF = MI->getParent()->getParent();

    PseudoMI = MF->CreateMachineInstr(HMCVEXII->get(HMCVEX::EXTIMM),
                                          MI->getDebugLoc());
...
void HMCVEXPacketizerList::reserveResourcesForLongImmediate (MachineBasicBlock::iterator MI) {
...
MachineBasicBlock::iterator HMCVEXPacketizerList::addToPacket(MachineInstr *MI) {
    ...
    do {
        ...
        if (Dependence) {
            ...
            MachineInstr* NOP = BuildMI(*MBB, MI, MI->getDebugLoc(), HMCVEXII->get(HMCVEX::NOP));
...
// as EHLabel. Though
bool HMCVEXPacketizerList::isSoloInstruction(MachineInstr *MI) {
...
bool HMCVEXPacketizerList::ignorePseudoInstruction(MachineInstr *MI,
...
bool HMCVEXPacketizerList::isLegalToPacketizeTogether(SUnit *SUI, SUnit *SUJ) {
...
void HMCVEXPacketizerList::AdvanceCycle() {
...
void HMCVEXPacketizerList::clearHazardTable() {
    DataHazards.clear();
}

bool HMCVEXPacketizer::runOnMachineFunction(MachineFunction &MF) {
    ...
    HMCVEXPacketizerList Packetizer(TM, MF, MLI);
...
//===----------------------------------------------------------------------===//
FunctionPass *llvm::createHMCVEXPacketizer(bool EnableVLIWScheduling, HMCVEXTargetMachine &TM) {
    return new HMCVEXPacketizer(TM, EnableVLIWScheduling);
}

char HMCVEXPacketizer::ID = 0;
//static RegisterPass<HMCVEXPacketizer> X("EnableVLIWScheduling", "Enable VLIW Scheduling");
