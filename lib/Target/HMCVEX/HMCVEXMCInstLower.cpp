//===-- HMCVEXMCInstLower.cpp - Convert HMCVEX MachineInstr to MCInst ---------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains code to lower HMCVEX MachineInstrs to their corresponding
// MCInst records.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "HMCVEX-mcinslower"

using namespace llvm;

HMCVEXMCInstLower::HMCVEXMCInstLower(HMCVEXAsmPrinter &asmprinter)
: AsmPrinter(asmprinter) {
    DEBUG(errs() << "MCInstLower\n");
}

void HMCVEXMCInstLower::Initialize(MCContext *C){
    Ctx = C;
}
...
// LowerOperand
MCOperand HMCVEXMCInstLower::LowerOperand(const MachineOperand &MO,
...
void HMCVEXMCInstLower::Lower(const MachineInstr *MI,
                           MCInst &OutMI,
                           MCInst &InBundleMI,
                           bool isInsideBundle,
    ...
    if (!MI) {
        InBundleMI.setOpcode(HMCVEX::NOPInstr0 + numValArgumentOrLane);
        MCOperand MCOp1 = MCOperand::createReg(HMCVEX::Reg0);
        InBundleMI.addOperand(MCOp1);
        MCOperand MCOp2 = MCOperand::createReg(HMCVEX::Reg0);
