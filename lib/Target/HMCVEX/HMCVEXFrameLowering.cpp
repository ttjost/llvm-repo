//===-- HMCVEXFrameLowering.cpp - HMCVEX Frame Information --------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the HMCVEX implementation of TargetFrameLowering class.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "HMCVEX-framelower"

// ---------------------------------------------------------------------------
//                                  FIXME
// ---------------------------------------------------------------------------
//  Verify this part to describe with precision how Prologue, Epilogue
//  and CALL should be implemented. Fix this in order to describe HMCVEX properly.
...
bool HMCVEXFrameLowering::hasReservedCallFrame(const MachineFunction &MF) const {
...
int HMCVEXFrameLowering::getFrameIndexOffset(const MachineFunction &MF,
                                             int FI) const {
...
void HMCVEXFrameLowering::emitPrologue(MachineFunction &MF,
    ...
    const HMCVEXInstrInfo &TII = *static_cast<const HMCVEXInstrInfo *>(MF.getSubtarget().getInstrInfo());
    const HMCVEXRegisterInfo *RegInfo = static_cast<const HMCVEXRegisterInfo *>(MF.getSubtarget().getRegisterInfo());
    ...
    if (StackSize == 0 && !MFI->adjustsStack() && !MFI->hasCalls() && !HMCVEXFI->isVarArgFunction()) return;
    ...
    if (HMCVEXFI->isVarArgFunction())
        StackSize -= 12;
    MFI->setStackSize(StackSize);
    
    TII.adjustStackPtr(HMCVEXFI, HMCVEX::Reg1, -RoundUpToAlignment(StackSize, 32), MBB, MBBI);
...
//        NumBytes = FrameSize - HMCVEXFI

//    }
    
}

void HMCVEXFrameLowering::emitEpilogue(MachineFunction &MF,
                                       MachineBasicBlock &MBB) const {
...
//    
////    if(StackSize){
////        BuildMI(MBB, MBBI, dl, TII.get(HMCVEX::RET)).addReg(HMCVEX::Reg1).addReg(HMCVEX::Reg1).addImm(MFI->getStackSize()).addReg(HMCVEX::Lr);
////    }else{
////        BuildMI(MBB, MBBI, dl, TII.get(HMCVEX::RET)).addReg(HMCVEX::Reg1).addReg(HMCVEX::Reg1).addImm(0).addReg(HMCVEX::Lr);
////    }
////    MBB.erase(MBBI);
...
bool
HMCVEXFrameLowering::spillCalleeSavedRegisters(MachineBasicBlock &MBB,
...
//        bool IsRAAndRetAddrIsTaken = Reg != HMCVEX::Lr;
...
bool
HMCVEXFrameLowering::restoreCalleeSavedRegisters(MachineBasicBlock &MBB,
                                                 MachineBasicBlock::iterator MI,
                                                 const std::vector<CalleeSavedInfo> &CSI,
                                                 const TargetRegisterInfo *TRI) const {
...
//        assert(Reg != HMCVEX::Lr && "Link register is always handled in emitEpilogue");
...
void HMCVEXFrameLowering::
eliminateCallFramePseudoInstr(MachineFunction &MF, MachineBasicBlock &MBB,
                              MachineBasicBlock::iterator I) const {
  MachineInstr &MI = *I;

  if (MI.getOpcode() == HMCVEX::ADJCALLSTACKDOWN) {
    // TODO: add code
  } else if (MI.getOpcode() == HMCVEX::ADJCALLSTACKUP) {
    // TODO: add code
  } else {
...
//// on being added.
//void
//HMCVEXFrameLowering::processFunctionBeforeCalleeSavedScan
...
//      MRI.setPhysRegUsed(HMCVEX::Lr);
//    else {
//      MRI.setPhysRegUnused(HMCVEX::Lr);
//    }
//}

bool HMCVEXFrameLowering::hasFP(const MachineFunction &MF) const{
    return false;
//    const MachineFrameInfo *MFI = MF.getFrameInfo();
