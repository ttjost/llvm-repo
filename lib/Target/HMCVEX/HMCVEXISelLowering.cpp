//===-- HMCVEXISelLowering.cpp - HMCVEX DAG Lowering Implementation -----------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines the interfaces that HMCVEX uses to lower LLVM code into a
// selection DAG.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "HMCVEX-lower"

extern cl::opt<bool> EnableSPMs;

#if 0
SDValue HMCVEXTargetLowering::getTargetNode(ConstantPoolSDNode *N, EVT Ty,
                                          SelectionDAG &DAG,
                                          unsigned Flag) const {
...
const char *HMCVEXTargetLowering::getTargetNodeName(unsigned Opcode) const {

    switch (Opcode) {

        case HMCVEXISD::WRAPPER:       return "HMCVEXISD::WRAPPER";
        case HMCVEXISD::PSEUDO_RET:    return "HMCVEXISD::PSEUDO_RET";
        case HMCVEXISD::PSEUDO_CALL:   return "HMCVEXISD::PSEUDO_CALL";
        case HMCVEXISD::PSEUDO_TAILCALL:   return "HMCVEXISD::PSEUDO_TAILCALL";


        case HMCVEXISD::MAX:           return "HMCVEXISD::MAX";
        case HMCVEXISD::MAXU:          return "HMCVEXISD::MAXU";
        case HMCVEXISD::MIN:           return "HMCVEXISD::MIN";
        case HMCVEXISD::MINU:          return "HMCVEXISD::MINU";

        case HMCVEXISD::ADDCG:         return "HMCVEXISD::ADDCG";
        case HMCVEXISD::DIVS:          return "HMCVEXISD::DIVS";

        case HMCVEXISD::ORC:           return "HMCVEXISD::ORC";
        case HMCVEXISD::SH1ADD:        return "HMCVEXISD::SH1ADD";
        case HMCVEXISD::SLCT:          return "HMCVEXISD::SLCT";

        case HMCVEXISD::MPYLL:         return "HMCVEXISD::MPYLL";
        case HMCVEXISD::MPYLLU:        return "HMCVEXISD::MPYLLU";
        case HMCVEXISD::MPYLH:         return "HMCVEXISD::MPYLH";
        case HMCVEXISD::MPYLHU:        return "HMCVEXISD::MPYLHU";
        case HMCVEXISD::MPYHH:         return "HMCVEXISD::MPYHH";
        case HMCVEXISD::MPYHHU:        return "HMCVEXISD::MPYHHU";
        case HMCVEXISD::MPYL:          return "HMCVEXISD::MPYL";
        case HMCVEXISD::MPYLU:         return "HMCVEXISD::MPYLU";
        case HMCVEXISD::MPYH:          return "HMCVEXISD::MPYH";
...
//@HMCVEXTargetLowering
HMCVEXTargetLowering::HMCVEXTargetLowering(const TargetMachine &TM,
                                     const HMCVEXSubtarget &STI)
...
//    setOperationAction(ISD::DYNAMIC_STACKALLOC, MVT::i32,  Expand);
    ...
    setStackPointerRegisterToSaveRestore(HMCVEX::Reg1);
    //setStackPointerRegisterToSaveRestore(HMCVEX::Reg1);
...
SDValue HMCVEXTargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
...
HMCVEXTargetLowering::getConstraintType(const std::string &Constraint) const {
...
HMCVEXTargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
    ...
    if (Constraint.size() == 1) {
        ...
        switch (Constraint[0]) {
            ...
            case 'r':   // GENERAL_REGS
                if (VT == MVT::i32)
                    return std::make_pair(0U, &HMCVEX::GPRegsRegClass);
                return std::make_pair(0U, &HMCVEX::BrRegsRegClass);
        }
    }
...
bool HMCVEXTargetLowering::isOffsetFoldingLegal(const GlobalAddressSDNode *GA) const {
    // The HMCVEX Target isn't yet aware of offsets.
    return false;
}
...
#include "HMCVEXGenCallingConv.inc"
...
static void AnalyzeRetResult(CCState &State,
    ...
    State.AnalyzeCallResult(Ins, RetCC_HMCVEX);
}

static void AnalyzeRetResult(CCState &State,
                             const SmallVectorImpl<ISD::OutputArg> &Outs){
    State.AnalyzeReturn(Outs, RetCC_HMCVEX);
}
...
bool HMCVEXTargetLowering::IsEligibleForTailCallOptimization(SDValue Callee,
...
SDValue
HMCVEXTargetLowering::LowerRETURNADDR(SDValue Op, SelectionDAG &DAG) const {
    const HMCVEXRegisterInfo *TRI = Subtarget.getRegisterInfo();
...
SDValue
HMCVEXTargetLowering::LowerFRAMEADDR(SDValue Op, SelectionDAG &DAG) const {
    const HMCVEXRegisterInfo *TRI = Subtarget.getRegisterInfo();
...
SDValue
HMCVEXTargetLowering::LowerCall(CallLoweringInfo &CLI,
    ...
    HMCVEXFunctionInfo *FuncInfo = MF.getInfo<HMCVEXFunctionInfo>();
//    bool IsTailCall = false;
    ...
    auto *TFL = static_cast<const HMCVEXFrameLowering *>(Subtarget.getFrameLowering());

    // If need, arguments on stack should be place after ScratchPad Area
    //ArgCCInfo.AllocateStack(TFL->getScratchArea(), 1);

    ArgCCInfo.AnalyzeCallOperands(Outs, CC_HMCVEX_Address);
...
//        llvm_unreachable("Target does not yet support Tail Calls.");
    ...
    SDValue StackPtr =
            DAG.getCopyFromReg(Chain, DL, HMCVEX::Reg1, getPointerTy(DLayout));;
    ...
    if (auto *G = dyn_cast<GlobalAddressSDNode>(Callee)) {
        ...
        Callee = DAG.getNode(HMCVEXISD::WRAPPER, DL, PtrVT, Callee);
...
//    // Glue the call to the argument copies, if any.
    ...
    if (IsTailCall) {
        ...
        return DAG.getNode(HMCVEXISD::PSEUDO_TAILCALL, DL, NodeTys, Ops);
    }

    Chain = DAG.getNode(HMCVEXISD::PSEUDO_CALL, DL, NodeTys, Ops);
    Glue = Chain.getValue(1);
...
void HMCVEXTargetLowering::passByValArg(SDValue Chain, SDLoc DL,
                                      SmallVector<std::pair<unsigned, SDValue>, 10> RegsToPass,
    ...
    if (NumRegs) {
        const ArrayRef<MCPhysReg> ArgRegs = { HMCVEX::Reg3, HMCVEX::Reg4, HMCVEX::Reg5, HMCVEX::Reg6, HMCVEX::Reg7, HMCVEX::Reg8, HMCVEX::Reg9, HMCVEX::Reg10 };
...
void HMCVEXTargetLowering::copyByValRegs(SDValue Chain, SDLoc DL, std::vector<SDValue> &OutChains, SelectionDAG &DAG,
    ...
    int FrameObjOffset;
    ArrayRef<MCPhysReg> ByValArgRegs = { HMCVEX::Reg3, HMCVEX::Reg4, HMCVEX::Reg5, HMCVEX::Reg6, HMCVEX::Reg7, HMCVEX::Reg8, HMCVEX::Reg9, HMCVEX::Reg10 };
...
void HMCVEXTargetLowering::writeVarArgRegs(std::vector<SDValue> &OutChains,
    ...
    MachineFunction &MF = DAG.getMachineFunction();
    MachineFrameInfo *MFI = MF.getFrameInfo();
    HMCVEXFunctionInfo *HMCVEXFI = MF.getInfo<HMCVEXFunctionInfo>();
...
//    if (ArgRegs.size() == Idx)
    ...
    HMCVEXFI->setVarArgsFrameIndex(FI);
...
void HMCVEXTargetLowering::HandleByVal(CCState *State, unsigned &Size,
    ...
    if (State->getCallingConv() != CallingConv::Fast) {
        unsigned RegSizeInBytes = 4;
        const ArrayRef<MCPhysReg> IntArgRegs = { HMCVEX::Reg3, HMCVEX::Reg4, HMCVEX::Reg5, HMCVEX::Reg6, HMCVEX::Reg7, HMCVEX::Reg8, HMCVEX::Reg9, HMCVEX::Reg10 };
...
//
SDValue
HMCVEXTargetLowering::LowerFormalArguments(SDValue Chain,
...
//        llvm_unreachable("Variable number of arguments not yet implemented!");
    ...
    CCInfo.AnalyzeFormalArguments(Ins, CC_HMCVEX_Address);
...
//===----------------------------------------------------------------------===//
SDValue
HMCVEXTargetLowering::LowerReturn(SDValue Chain,
                               CallingConv::ID CallConv,
                               bool IsVarArg,
                               const SmallVectorImpl<ISD::OutputArg> &Outs,
                               const SmallVectorImpl<SDValue> &OutVals,
...
//    // The HMCVEX ABIs for returning structs by value requires that we copy
//    // the sret argument into $v0 for the return. We saved the argument into
//    // a virtual register in the entry block, so now we copy the value out
//    // and into $v0.
//    if (MF.getFunction()->hasStructRetAttr()) {
//      HMCVEXFunctionInfo *VInfo = MF.getInfo<HMCVEXFunctionInfo>();
//      unsigned Reg = VInfo->getSRetReturnReg();

//      if (!Reg)
//        llvm_unreachable("sret virtual register not created in the entry block");
//      SDValue Val = DAG.getCopyFromReg(Chain, DL, Reg, getPointerTy(DLayout));
//      unsigned Reg3 = HMCVEX::Reg3;
...
//    }
    ...
    return DAG.getNode(HMCVEXISD::PSEUDO_RET, DL, MVT::Other, RetOps);
}

SDValue HMCVEXTargetLowering::LowerGlobalAddress(SDValue Op, SelectionDAG &DAG) const{
    ...
    return DAG.getNode(HMCVEXISD::WRAPPER, SDLoc(Op),
                       getPointerTy(DLayout), Result);

}

SDValue HMCVEXTargetLowering::LowerExternalSymbol(SDValue Op, SelectionDAG &DAG) const{
    ...
    return DAG.getNode(HMCVEXISD::WRAPPER, dl,
                       getPointerTy(DLayout), Result);
}

SDValue
HMCVEXTargetLowering::LowerVASTART(SDValue Op, SelectionDAG &DAG) const {
    // VASTART stores the address of the VarArgsFrameIndex slot into the
    // memory location argument.
    MachineFunction &MF = DAG.getMachineFunction();
    HMCVEXFunctionInfo *QFI = MF.getInfo<HMCVEXFunctionInfo>();
...
SDValue HMCVEXTargetLowering::LowerConstant(SDValue Op, SelectionDAG &DAG) const {
...
//SDValue HMCVEXTargetLowering::LowerSETCC(SDValue Op, SelectionDAG &DAG) const {
...
SDValue HMCVEXTargetLowering::LowerLOAD(SDValue Op, SelectionDAG &DAG) const {
...
SDValue HMCVEXTargetLowering::LowerSTORE(SDValue Op, SelectionDAG &DAG) const {
...
SDValue HMCVEXTargetLowering::LowerADDSUBWithFlags(SDValue Op, SelectionDAG &DAG) const {
    ...
    if (CinFlag) {
        ...
        return DAG.getNode(HMCVEXISD::ADDCG, dl, VTs, lhs, rhs, Op3);
    } else {
...
// HMCVEX does not natively support 64 bit instructions, so we should
// not worry about the upper part of the result.
SDValue HMCVEXTargetLowering::LowerMUL(SDValue Op, SelectionDAG &DAG) const {
    ...
    if (ValueType == MVT::i32){
        ...
        Opc1 = HMCVEXISD::MPYLU;
        Opc2 = HMCVEXISD::MPYHS;
    }else{
        DEBUG(errs() << "ISD::MUL with MVT::i16");
        return DAG.getNode(HMCVEXISD::MPYLL, dl, ValueType, lhs, rhs);
    }
...
SDValue HMCVEXTargetLowering::LowerMULHS(SDValue Op, SelectionDAG &DAG) const {
    ...
    if (ConstantSDNode *C = dyn_cast<ConstantSDNode>(RHS)) {
        ...
        if(C->getConstantIntValue()->getBitWidth() > 9) {
            SDValue v = DAG.getNode(HMCVEXISD::MOV, dl, MVT::i32, RHS);
...
SDValue HMCVEXTargetLowering::LowerMULHU(SDValue Op, SelectionDAG &DAG) const {
...
SDValue HMCVEXTargetLowering::LowerSDIV(SDValue Op, SelectionDAG &DAG) const {
    ...
    SDValue Reg0 = DAG.getRegister(HMCVEX::Reg0, MVT::i32);
...
//                                Reg0, Reg0, CondCodeBr);
    ...
    ADDCGNodes.push_back(DAG.getNode(HMCVEXISD::ADDCG, DL,
                                     VTList,
    ...
    DIVSNodes.push_back(DAG.getNode(HMCVEXISD::DIVS, DL,
                                    VTList,
                                    Reg0, SlctDivisor,
    ...
    SDValue ORC = DAG.getNode(HMCVEXISD::ORC, DL, MVT::i32,
    ...
    SDValue SH1ADDNode = DAG.getNode(HMCVEXISD::SH1ADD, DL,
                                     MVT::i32, ORC,
                                     CmpRes);
...
SDValue HMCVEXTargetLowering::LowerUDIV(SDValue Op, SelectionDAG &DAG) const {
...
SDValue HMCVEXTargetLowering::LowerSREM(SDValue Op, SelectionDAG &DAG) const {
...
SDValue HMCVEXTargetLowering::LowerUREM(SDValue Op, SelectionDAG &DAG) const {
...
SDValue HMCVEXTargetLowering::CombineMinMaxLegacy(SDLoc DL, EVT VT, SDValue lhs, SDValue rhs,
    ...
    switch (CCOpcode) {
        case ISD::SETULE:
        case ISD::SETULT: {
            unsigned Opc = (lhs == True) ? HMCVEXISD::MINU : HMCVEXISD::MAXU;
            return DAG.getNode(Opc, DL, VT, lhs, rhs);
        }
        case ISD::SETLE:
        case ISD::SETLT: {
            unsigned Opc = (lhs == True) ? HMCVEXISD::MIN : HMCVEXISD::MAX;
            return DAG.getNode(Opc, DL, VT, lhs, rhs);
        }
        case ISD::SETGT:
        case ISD::SETGE: {
            unsigned Opc = (lhs == True) ? HMCVEXISD::MAX : HMCVEXISD::MIN;
            return DAG.getNode(Opc, DL, VT, lhs, rhs);
        }
        case ISD::SETUGE:
        case ISD::SETUGT: {
            unsigned Opc = (lhs == True) ? HMCVEXISD::MAXU : HMCVEXISD::MINU;
            return DAG.getNode(Opc, DL, VT, lhs, rhs);
        }
        default:
            return SDValue();
...
SDValue HMCVEXTargetLowering::PerformDAGCombine(SDNode *N,
