//===-- HMCVEXAsmPrinter.cpp - HMCVEX LLVM Assembly Printer -------------------===//
//
...
// of machine-dependent LLVM code to GAS-format HMCVEX assembly language.
//
//===----------------------------------------------------------------------===//

#include "HMCVEXAsmPrinter.h"

#include "InstPrinter/HMCVEXInstPrinter.h"

#include "MCTargetDesc/HMCVEXBaseInfo.h"
...
// FIXME :  Verify if this is correct for our HMCVEX architecture

bool HMCVEXAsmPrinter::runOnMachineFunction(MachineFunction &MF){
    Subtarget = &MF.getSubtarget<HMCVEXSubtarget>();

    DEBUG(errs() << "\n\nHMCVEX asm Printer\n\n");
    HMCVEXFI = MF.getInfo<HMCVEXFunctionInfo>();
...
void HMCVEXAsmPrinter::EmitInstruction(const MachineInstr *MI){
    ...
    if (MI->isBundle()) {
        ...
        if (PrintNops) {
            ...
            TmpInst0.setOpcode(HMCVEX::BUNDLE);
...
void HMCVEXAsmPrinter::printSavedRegsBitmask(raw_ostream &O){
    ...
    //unsigned CPURegSize = HMCVEX::B

    unsigned i = 0, e = CSI.size();
    // Set CPU Bitmask
    for (; i != e; ++i){
        unsigned Reg = CSI[i].getReg();
        unsigned RegNum = getHMCVEXRegisterNumbering(Reg);
        CPUBitmask |= (1 << RegNum);
    }
...
void HMCVEXAsmPrinter::printHex32(unsigned Value, raw_ostream &O) {
  O << "0x";
  for (int i = 7; i >= 0; i--)
...
//	.set	nomacro
/// Frame Directive
void HMCVEXAsmPrinter::emitFrameDirective(){
...
//                                StringRef(HMCVEXInstPrinter::getRegisterName(stackReg)).lower() +
//                                "," + Twine(stackSize) + ", $" +
//                                StringRef(HMCVEXInstPrinter::getRegisterName(returnReg)).lower());
}

const char *HMCVEXAsmPrinter::getCurrentABIString() const {
    return "abi32";
}

// .entry caller, sp=$r0.1, rl=$l0.0, asize=80, arg()
// main:
void HMCVEXAsmPrinter::EmitFunctionEntryLabel() {
//    if(OutStreamer.hasRawTextSupport())
...
void HMCVEXAsmPrinter::EmitFunctionBodyStart() {
...
///
/// .endp
void HMCVEXAsmPrinter::EmitFunctionBodyEnd() {
...
//  .previous
void HMCVEXAsmPrinter::EmitStartOfAsmFile(Module &M){
...
void HMCVEXAsmPrinter::EmitEndOfAsmFile(Module &M) {
...
MachineLocation HMCVEXAsmPrinter::getDebugValueLocation(const MachineInstr *MI) const{
    //Handles frame address emit in HMCVEXInstrInfo::emitFrameIndexDebugValue.
...
void HMCVEXAsmPrinter::PrintDebugValueComment(const MachineInstr *MI,
                                           raw_ostream &OS) {
  // TODO: implement
...
bool HMCVEXAsmPrinter::PrintAsmOperand(const MachineInstr *MI, unsigned OpNo,
    ...
    switch (MO.getType()) {
        ...
        case MachineOperand::MO_Register:
            O << "$" << StringRef(HMCVEXInstPrinter::getRegisterName(MO.getReg())).lower();
            return false;
    }
...
extern "C" void LLVMInitializeHMCVEXAsmPrinter() {
  RegisterAsmPrinter<HMCVEXAsmPrinter> X(TheHMCVEXTarget);
  RegisterAsmPrinter<HMCVEXAsmPrinter> Y(TheHMCVEXNewTarget);
}
